...
async function http(
  ...
  for (let i = 0; i < value.retries; i++) {
    try {
      ...
      console.log(
        `HTTP check for ${key} failed (attempt ${i} of ${value.retries} every ${value.timeout} seconds): ${error}`
      )
    }
    await new Promise(resolve => setTimeout(resolve, value.timeout * 1000)) // and retry every value.timeout seconds
    if (i === value.retries - 1) {
      // if we've reached the last attempt
